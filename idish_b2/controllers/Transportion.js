        ...
        for (const item of products) {
            ...
            if (unit === "box_quantity") {
                ...
                if (product.isPackage) {
                    product.package_quantity -= quantity * product.package_quantity_per_box;
                    product.quantity -= quantity * product.package_quantity_per_box * product.quantity_per_package;
                } else {
                ...
                if (product.isPackage) {
                    ...
                    product.box_quantity -= quantity / (product.quantity_per_package * product.package_quantity_per_box);
                }
            }
...
exports.acceptController = async (req, res) => {
    try {
        ...
        const transportion = await Transportion.findById(transportion_id).populate('products.product_id');
        if (!transportion) {
            ...
            if (product) {
                ...
                if (unit === "box_quantity") {
                    ...
                    if (baseProduct.isPackage) {
                        newProduct.package_quantity = quantity * baseProduct.package_quantity_per_box;
                        newProduct.quantity = newProduct.package_quantity * baseProduct.quantity_per_package;
                    } else {
                    ...
                    if (baseProduct.isPackage) {
                        newProduct.package_quantity = quantity / baseProduct.quantity_per_package;
                        newProduct.box_quantity = newProduct.package_quantity / baseProduct.package_quantity_per_box;
                    }
                }
        ...
        res.status(200).json({ message: "Jo'natma muvaffaqiyatli qabul qilindi", data: transportion });
...
exports.rejectTransportion = async (req, res) => {
    try {
        ...
        for (const item of transportion.products) {
            ...
            if (!product) {
                return res.status(404).json({ message: `Mahsulot topilmadi: ${product_id._id}` });
            }
        ...
        res.status(200).json({ message: "Jo'natma bekor qilindi va mahsulotlar qayta qo‘shildi", data: transportion });
