...
const ReconciliationAct = () => {
  ...
  const filteredSales = useMemo(() => {
    return sales.filter(item => {
      ...
      if (selectedClient) return item.clientId?._id === selectedClient && matchDate;
  ...
  const filteredDebts = useMemo(() => {
    return debts.filter(item => {
      const matchDate = isInDateRange(item.createdAt);
      if (selectedClient) return item.clientId?._id === selectedClient && matchDate;
      if (selectedPartner) return item.partnerId === selectedPartner && matchDate;
      return false;
    });
  }, [debts, selectedClient, selectedPartner, startDate, endDate]);

  const filteredPartnerProducts = useMemo(() => {
    return partnerProducts.filter(item =>
  ...
  return (
    <div className="act" style={{ padding: 20 }}>
      <div style={{ display: "flex", gap: 20, marginBottom: 20 }}>
        ...
        <Button disabled={!selectedClient && !selectedPartner} type="primary" onClick={printPDF}>
          Chop etish
        </Button>
        ...
        <>
          <Table
            ...
            columns={[
              ...
              { title: "Birlik", dataIndex: "unit", render: (unit) => quantityText[unit] || unit },
